...
from pendulum_dynamics import (
    generate_sinusoidal_torque,
    simple_pendulum_dynamics_torqued,
...
def prepare_csv_dict(theta_wrapped_rad_subset, omega_subset,
                     omega_dot_subset, tau_pendulum_subset):
    """Prepares a dictionary suitable for saving time-series data (excluding mass/gravity matrices) to CSV."""
    csv_subset_dict = {
        'q_1': theta_wrapped_rad_subset,       # Pendulum angle (wrapped)
        'dq_1': omega_subset,                  # Pendulum angular velocity
        'ddq_1': omega_dot_subset,             # Pendulum angular acceleration
        'output_1': tau_pendulum_subset,      # Generalized force on theta (pendulum torque, usually 0)
    }
...
def main():
    ...
    if args.load:
        ...
        force_profile = generate_sinusoidal_torque(args.force_sines, args.force_max_amp, args.force_max_freq)
        print("Running simulation with external force...")
        theta_0_rad = np.radians(args.init_theta_deg)
        q0 = np.array([theta_0_rad, 0.0]) # [x, x_dot, theta, omega]
        ...
        def dynamics_for_solver(t_curr, q_curr):
            return simple_pendulum_dynamics_torqued(t_curr, q_curr, m_param, l_param, g_const_param, force_profile)
        ...
        theta_sim = q_sol[:, 0]; omega = q_sol[:, 1]

        print("Calculating accelerations, forces, mass matrix, and gravity vector...")
        num_points = len(t)
        omega_dot = np.zeros(num_points)
        tau_pendulum_applied = np.zeros(num_points)
        mass_matrix_seq = np.zeros((num_points, 1, 1)); gravity_vector_seq = np.zeros((num_points, 1))

        for i in range(num_points):
            derivs = simple_pendulum_dynamics_torqued(t[i], q_sol[i, :],  m_param, l_param, g_const_param, force_profile)
            omega_dot[i] = derivs[1]
            tau_pendulum_applied[i] = force_profile(t[i])
            mass_matrix_seq[i] = calculate_mass_matrix(q_sol[i,1], M_param, m_param, l_param)
            gravity_vector_seq[i] = calculate_gravity_vector(q_sol[i, 1], m_param, l_param, g_const_param)
    ...
    if args.split:
        try:
            ...
            required_vars_for_split_main = {
                ...
                'omega_dot': omega_dot,
                 'tau_pendulum_applied': tau_pendulum_applied
            }
                ...
                if PANDAS_AVAILABLE:
                    try:
                        test_csv_dict_main = prepare_csv_dict(
                            ...
                            theta_sim_wrapped[test_indices], omega[test_indices],omega_dot[test_indices],
    ...
    if not args.no_plot:
        ...
        if all(v is not None for v in [t,theta_sim_wrapped, omega]):
            plot_simple_pendulum_results(t, theta_sim_wrapped, omega,omega_dot, tau_pendulum_applied,save_dir=args.save_dir)
            print("Static plot window generated.")
            
        else:
            print("Skipping static plots due to missing essential data (t, x, theta_sim_wrapped, omega).")
    ...
    if args.animate:
        ...
        if all(v is not None for v in [t, x, theta_sim]):
            animate_simple_pendulum(t, x, theta_sim, anim_params, anim_dt)
