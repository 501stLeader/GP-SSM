...
def get_K_blocks_GIP_sum_PANDA_5dof_no_subs(X1, X2, pos_indices, vel_indices, acc_indices, sigma_kin_vel_list, sigma_kin_pos_list, sigma_pot_list):
    ...
    x24 = 1.0*x13*x18*x23
    x25 = sigma_pot_1_off + x0*x3 + x1*x2
    x26 = 1.0*x23*x25*x8
    
    K_block_list = []
    K_block_list.append(-x24*x8*(x0*x1 - x2*x3))
    K_block_list.append(-x24*x25*(-x4*x7 + x5*x6))
    K_block_list.append(-x18*x26*(x10*x11 - x12*x9))
    K_block_list.append(-x13*x26*(-x14*x17 + x15*x16))
    K_block_list.append(-1.0*x13*x18*x25*x8*(-x19*x22 + x20*x21))
