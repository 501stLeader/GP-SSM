...
def get_K_blocks_GIP_sum_PANDA_6dof_no_subs(X1, X2, pos_indices, vel_indices, acc_indices, sigma_kin_vel_list, sigma_kin_pos_list, sigma_pot_list):
    ...
    x29 = 1.0*x13*x18*x23*x28
    x30 = sigma_pot_1_off + x0*x3 + x1*x2
    x31 = 1.0*x23*x28*x30*x8
    x32 = 1.0*x13*x18*x30*x8
    
    K_block_list = []
    K_block_list.append(-x29*x8*(x0*x1 - x2*x3))
    K_block_list.append(-x29*x30*(-x4*x7 + x5*x6))
    K_block_list.append(-x18*x31*(x10*x11 - x12*x9))
    K_block_list.append(-x13*x31*(-x14*x17 + x15*x16))
    K_block_list.append(-x28*x32*(-x19*x22 + x20*x21))
    K_block_list.append(-x23*x32*(-x24*x27 + x25*x26))
