...
def get_K_blocks_GIP_sum_PANDA_3dof_no_subs(X1, X2, pos_indices, vel_indices, acc_indices, sigma_kin_vel_list, sigma_kin_pos_list, sigma_pot_list):
    ...
    x13 = 1.0*sigma_pot_3_off + 1.0*x10*x9 + 1.0*x11*x12
    x14 = sigma_pot_1_off + x0*x3 + x1*x2
    
    K_block_list = []
    K_block_list.append(-x13*x8*(x0*x1 - x2*x3))
    K_block_list.append(-x13*x14*(-x4*x7 + x5*x6))
    K_block_list.append(-1.0*x14*x8*(x10*x11 - x12*x9))
